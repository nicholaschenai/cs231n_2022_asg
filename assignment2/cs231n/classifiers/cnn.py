...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # divide by 2 cos pooling reduces size
    ...
    def loss(self, X, y=None):
        ...
        dw += self.reg * W3 # l2 regularization
        loss += 0.5 * self.reg * (W3**2).sum()
        grads["W3"], grads["b3"] = dw, db
        dx = relu_backward(dx, cache_list.pop())
        dx, dw, db = affine_backward(dx, cache_list.pop())
        dw += self.reg * W2 # l2 regularization
        loss += 0.5 * self.reg * (W2**2).sum()
        grads["W2"], grads["b2"] = dw, db
        dx, dw, db = conv_relu_pool_backward(dx, cache_list.pop())
        dw += self.reg * W1 # l2 regularization
