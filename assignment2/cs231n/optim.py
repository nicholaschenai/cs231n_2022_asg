...
def sgd_momentum(w, dw, config=None):
    ...
    # actually can just directly do next_w = w + v
...
def rmsprop(w, dw, config=None):
    ...
    # actually to unwrap vars, can do sth like 
...
def adam(w, dw, config=None):
    ...
    beta1 = config["beta1"]
    beta2 = config["beta2"]
    # see trick in rmsprop to unwrap variables
    ...
    # reshape cos sometimes get error size (100,) doesnt match (1,100)
    update = - config["learning_rate"] * mt / (np.sqrt(vt) + config["epsilon"])
