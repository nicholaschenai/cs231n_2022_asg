...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # can use torch.zeros_like and torch.ones_like
    real_labels = torch.ones(logits_real.size()).type(dtype).squeeze()
    fake_labels = torch.zeros(logits_fake.size()).type(dtype).squeeze()
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.ones(logits_fake.size()).type(dtype).squeeze()
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (((scores_real-1)**2).mean()+(scores_fake**2).mean())*0.5
