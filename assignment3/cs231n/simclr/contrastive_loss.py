...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.nn.functional.cosine_similarity(z_i, z_j, -1)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_d_k = torch.exp(torch.nn.functional.cosine_similarity(z_k, out, -1)/tau)
        sim_d_k[k] = 0
        sim_d_kN = torch.exp(torch.nn.functional.cosine_similarity(z_k_N, out, -1)/tau)
        sim_d_kN[k+N] = 0
        total_loss += -torch.log(sim_d_k[k+N]/sim_d_k.sum()) -torch.log(sim_d_k[k+N]/sim_d_kN.sum())
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pos_pairs = torch.nn.functional.cosine_similarity(out_left, out_right).reshape((len(out_left),1))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = torch.exp(sim_matrix/tau).to(device=device)
