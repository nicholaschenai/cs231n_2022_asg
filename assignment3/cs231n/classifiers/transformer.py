...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # note to self: past asgs give this ans by default
        we = self.positional_encoding(self.embedding(captions))
        ve = self.visual_projection(features).unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T,T))
        # print(we.shape, ve.shape, tgt_mask.shape)
        scores = self.output(self.transformer(we, ve, tgt_mask))
