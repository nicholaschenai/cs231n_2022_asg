...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
      dxt, dprev_h, dWxt, dWht, dbt = rnn_step_backward(dprev_h + dh[:,t,:], cache[t])
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # note to self: convert to one hot encoding so can do matmul so backprop easier
    # x becomes (N, T, V), then matrix multiply with W
    N, T = x.shape
    V, D = W.shape
    xPrime = np.zeros((N*T, V))
    xPrime[range(N*T),x.flatten()] = 1
    xPrime = xPrime.reshape((N,T,V))
    out = xPrime @ W
    cache = xPrime
    # elegant soln: out, cache = W[x], (x, W)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def word_embedding_backward(dout, cache):
    ...
    dW = np.einsum('ntv,ntd->vd', cache, dout)
    # alt soln
    # dW = np.zeros_like(W)
    # np.add.at(dW, x, dout)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # NOTE TO SELF: tensor outer product to simplify backprop eqns

    # lets work thru eqns
    # L = L_h + L_c. dL/dk = dL_h/d_next_h d_next_h/dk + dL_c/d_next_c d_next_c/dk
    # = dnext_h d_next_h/dk + dnext_c d_next_c/dk 
    # dnext_h (do/dk * tanh(next_c) + o*(1-tanh^2(next_c)) d_next_c/dk) + dnext_c d_next_c/dk
    # = dnext_h (do/dk) * tanh(next_c) + d_next_c/dk (dnext_h*o*(1-tanh^2(next_c)) + dnext_c)
    # look at first term. o depends on A which depends on x, Wx, Wh, prev_h, b

    # solve for first term
    # do/dk = do/da_0 da_0/dk = o*(1-o) * da_o/dk
    # A_ij = X_il Wx_lj + prev_h_im Wh_mj + np.ones((N,1)) @ b

    # ok for clarity, lets split Wx into Wx{ifog} and similarly Wh{ifog}, b{ifog}
    # these separate things are independent
    # da_o/dx = _ @ Wxo.T
    # da_o/dWxo = X.T @ _
    # da_o/dprev_h = _ @ Who.T
    # da_o/dWho = prev_h.T @ _
    # da_o/dbo = np.ones((1,N)) @ _

    # now focus on d_next_c/dk = df/dk * prev_c + f* d_prev_c/dk + di/dk * g + i*dg/dk
    # for derivatives of i, f, g wrt k, same as above o
    d_o_coeff = dnext_h * np.tanh(next_c) * o * (1-o)
    d_next_c_coeff = dnext_h*o*(1-np.tanh(next_c)**2) + dnext_c
    dprev_c = f * d_next_c_coeff
  
    di_mat = d_next_c_coeff * g * i * (1-i)
    df_mat = d_next_c_coeff * prev_c * f * (1-f)
    dg_mat = d_next_c_coeff * i * (1-g**2)
    difog_mat = np.concatenate((di_mat, df_mat, d_o_coeff, dg_mat), axis=1)

    dprev_h = difog_mat @ Wh.T
    dx = difog_mat @ Wx.T
    dWx = x.T @ difog_mat
    dWh = prev_h.T @ difog_mat
    db = np.ones((1,N)) @ difog_mat
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
      prev_h, prev_c, cache_t = lstm_step_forward(x[:,t,:], h[-1], prev_c, Wx, Wh, b)
      h.append(prev_h)
